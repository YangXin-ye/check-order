  ...
  <div>
    <el-form :inline="true" class="demo-form-inline">
      ...
      <el-form-item>
        ...
        <el-button type="primary" @click="exportAllData()">导出</el-button> <!-- Export Button -->
      </el-form-item>
    </el-form>
...
<script>
import { getAccountionRecords,exportAccountionRecords } from '@/api/accountionRecord';
...
export default {
  ...
  methods: {
    ...
    exportAllData() {
      ...
      exportAccountionRecords(params).then(response => {
        const blob = new Blob([response], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
        const url = window.URL.createObjectURL(blob)
        const link = document.createElement('a')
        link.href = url
        link.setAttribute('download', '记账数据导出.xlsx')
    ...
    getAccountionRecordList() {
      ...
      getAccountionRecords({ params }).then(res => {
